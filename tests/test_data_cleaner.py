...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        df = make_sample_df()
        cleaner = DataCleaner()
        cols_to_check = ["name", "age"]
        
     
        result = cleaner.drop_invalid_rows(df, cols_to_check)
        
    
        self.assertLess(len(result), len(df), "El número de filas debe ser menor después de eliminar valores faltantes.")
        
   
        self.assertEqual(result[cols_to_check].isna().sum().sum(), 0, "No deberían quedar valores NaN/None en las columnas especificadas.")
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        df_outlier = pd.DataFrame({"age": [10, 15, 20, 25, 100]})
        
        result = cleaner.remove_outliers_iqr(df_outlier, "age", factor=1.5)
        

        self.assertNotIn(100, result["age"].values, "El valor extremo (100) debería haber sido eliminado por IQR.")
        

        self.assertIn(10, result["age"].values, "El valor 10 no debería haber sido eliminado.")
        self.assertIn(25, result["age"].values, "El valor 25 no debería haber sido eliminado.")
        self.assertEqual(len(result), 4, "El DataFrame resultante debe tener 4 filas (solo se eliminó un outlier).")    
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        with self.assertRaises(KeyError, msg="Debe lanzar KeyError al intentar remover outliers de una columna no existente."):
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        with self.assertRaises(TypeError, msg="Debe lanzar TypeError al intentar remover outliers de una columna no numérica."):
