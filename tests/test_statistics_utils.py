...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        # Valores esperados: [(1+2)/2, (2+3)/2, (3+4)/2] = [1.5, 2.5, 3.5]
        expected = np.array([1.5, 2.5, 3.5])
        
        # Verificar que los valores son correctos (con tolerancia)
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7, err_msg="Los valores de la media móvil no son los esperados para el caso básico.")
        
        # Verificar la forma/dimension del array resultante
        self.assertEqual(result.shape, expected.shape, "La forma del array resultante es incorrecta.")
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        # 1. Verificar ValueError para window=0 (no positivo)
        with self.assertRaises(ValueError, msg="Debe lanzar ValueError si la ventana es cero o negativa."):
            utils.moving_average(arr, window=0)
            
        # 2. Verificar ValueError para window > len(arr)
        with self.assertRaises(ValueError, msg="Debe lanzar ValueError si la ventana es mayor que la longitud del array."):
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        # Verificar ValueError para secuencias multidimensionales
        with self.assertRaises(ValueError, msg="Debe lanzar ValueError para secuencias que no son 1D."):
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        # Verificar ValueError cuando la desviación estándar es cero
        with self.assertRaises(ValueError, msg="Debe lanzar ValueError si la desviación estándar es cero."):
            utils.zscore(arr)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        # min=2, max=6, range=4. Valores esperados: [0/4, 2/4, 4/4] = [0.0, 0.5, 1.0]
        expected = np.array([0.0, 0.5, 1.0])
        
        # Verificar que los valores transformados son correctos (con tolerancia)
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7, err_msg="Los valores escalados no coinciden con la transformación Min-Max esperada.")
        
        # Verificar que el mínimo del resultado es 0.0
        self.assertAlmostEqual(result.min(), 0.0, places=7, msg="El valor mínimo escalado no es 0.0.")
        
        # Verificar que el máximo del resultado es 1.0
        self.assertAlmostEqual(result.max(), 1.0, places=7, msg="El valor máximo escalado no es 1.0.")
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        # Verificar ValueError cuando todos los valores son iguales
        with self.assertRaises(ValueError, msg="Debe lanzar ValueError si todos los valores son constantes."):
